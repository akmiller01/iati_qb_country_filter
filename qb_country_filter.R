...
if(!file.exists(paste0(recipient_country,"_activities.RData"))){
  ...
  for(i in 1:(length(chunk_seq)-1)){
    ...
    tmp <- fread(act_filename,colClasses = c(rep("character", 344)))
    act_list[[i]] = tmp
  }
...
drop = names(act)[which(endsWith(names(act),"_xml"))]
drop = c(drop,"_version_","X_version_","id")
act[,drop] = NULL
...
write_excel_csv(all,paste0(recipient_country,"_transactions_merge_utf8.csv"),na="")
